...
struct database {
  std::map<osm_id_t, changeset> m_changesets;
...
void parse_info(element_info &info, const xmlChar **attributes) {
  info.id = get_attribute<osm_id_t>("id", 3, attributes);
  info.version = get_attribute<osm_id_t>("version", 8, attributes);
  info.changeset = get_attribute<osm_id_t>("changeset", 10, attributes);
...
struct xml_parser {
  ...
  static void start_element(void *ctx, const xmlChar *name,
    ...
    if (strncmp((const char *)name, "node", 5) == 0) {
      ...
      std::pair<std::map<osm_id_t, changeset>::iterator, bool> status =
      ...
      if (parser->m_cur_changeset != NULL) {
        ...
        info.author_id = get_attribute<osm_id_t>("uid", 4, attributes);
...
struct static_data_selection : public data_selection {
  ...
  virtual void write_changesets(output_formatter &formatter,
                                const pt::ptime &now) {
    BOOST_FOREACH(osm_id_t id, m_changesets) {
      std::map<osm_id_t, changeset>::iterator itr = m_db->m_changesets.find(id);
  ...
  virtual int select_changesets(const std::vector<osm_id_t> &ids) {
    int selected = 0;
    BOOST_FOREACH(osm_id_t id, ids) {
      std::map<osm_id_t, changeset>::iterator itr = m_db->m_changesets.find(id);
      if (itr != m_db->m_changesets.end()) {
...
private:
  ...
  std::set<osm_id_t> m_nodes, m_ways, m_relations, m_changesets;
#ifdef ENABLE_EXPERIMENTAL
